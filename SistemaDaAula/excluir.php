...
<body>
  <form method="post">
      <label>Informe o email para excluir:</label>
      <input type="text" name="txtemail" maxlength="45">
  ...
  if (!empty($_POST['btn-excluir'])){
    ...
    include 'conexao.php';
     $emailExcluir = $_POST['txtemail'];
    
        // Query SQL para deletar o registro
        $sqlExcluir = "DELETE FROM clientes WHERE email_cli = '$emailExcluir'";
    
        // Executa a query
        $result = $conn->query($sqlExcluir);
